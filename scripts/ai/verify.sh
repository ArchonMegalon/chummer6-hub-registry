...
if rg -n '<HintPath>.*Chummer\.Hub\.Registry\.Contracts.*bin' "$repo_root/Chummer.Run.Registry/Chummer.Run.Registry.csproj" >/dev/null; then
...
python3 "$repo_root/scripts/verify_public_release_channel.py" /tmp/chummer-hub-registry-release-channel.json
python3 "$repo_root/scripts/verify_public_release_channel.py" /tmp/chummer-hub-registry-release-channel-compat.json
python3 - <<'PY'
import json
