...
EXPECTED_DO_NOT_REOPEN_REASON = (
    "M144.2 chummer6-hub-registry release-channel tuple coverage and startup-smoke receipt identity are complete; "
    "future shards must verify the local mirror row, queue row, closeout, published tuple proof, and verifier-routing guard "
    "instead of reopening the registry proof-drift slice."
)
EXPECTED_PROOF = [
    "/docker/chummercomplete/chummer-hub-registry/docs/RELEASE_CHANNEL_PIPELINE.md",
    "/docker/chummercomplete/chummer-hub-registry/docs/next90-m144-registry-release-tuple-proof.closeout.md",
    ...
    "/docker/chummercomplete/chummer-hub-registry/scripts/verify_next90_m144_registry_release_tuple_proof.py",
    "/docker/chummercomplete/chummer-hub-registry/scripts/test_verify_next90_m144_registry_release_tuple_proof.py",
...
REQUIRED_PIPELINE_SNIPPETS = (
    ...
    "Repo-local verification for this proof lane must route through `scripts/ai/verify.sh`, which runs `scripts/verify_public_release_channel.py .codex-studio/published` and `scripts/verify_next90_m144_registry_release_tuple_proof.py`",
)
...
def verify_successor_registry(path: Path) -> None:
    ...
    required = (
        ...
        "status: complete",
        "evidence:",
        "scripts/verify_public_release_channel.py keeps promoted tuple coverage, startup-smoke receipt identity, and receipt freshness fail-closed against the published release-channel bundle",
        "scripts/verify_next90_m144_registry_release_tuple_proof.py plus /docker/chummercomplete/chummer-hub-registry/scripts/test_verify_next90_m144_registry_release_tuple_proof.py keep the local successor mirror row, queue row, closeout, published proof, and verifier-routing hook executable",
        "scripts/ai/verify.sh now routes through the published-bundle verifier and the M144 package verifier before repo builds/tests run",
        "docs/next90-m144-registry-release-tuple-proof.closeout.md records the shipped proof anchors",
    )
...
def verify_manifest(path: Path, *, label: str) -> None:
    ...
    if coverage.get("complete") is not True:
        fail(f"{label} desktopTupleCoverage.complete must be true")
    for key in (
        ...
        if value != []:
            fail(f"{label} desktopTupleCoverage.{key} must be []")
