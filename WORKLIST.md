...
- [in_progress] Close the registry share of `E2` by proving publication, installs, reviews, discovery, compatibility, and runtime-bundle-head projections are authoritative registry read models rather than advisory adjuncts. Evidence now lives in `docs/REGISTRY_PRODUCT_READMODELS.md` plus `Chummer.Run.Registry.Verify/Program.cs` controller-backed assertions for create/search/preview/install/review/projection/publication/moderation/runtime-head flows.
- [in_progress] Close the registry share of `E2b` by proving operator/projection consumers stay read-only, bounded, and downstream of canonical registry state. Current proof is the registry-owned projection plane (`GetPipelineProjection`, projection/search/install/head lookups, publication audit/timeline projections) and the explicit consumer rule in `docs/REGISTRY_PRODUCT_READMODELS.md`; richer downstream consumer evidence is still open.
- [queued] Close the registry share of `F1` by publishing restore/runbook/drill evidence for artifact metadata, publication state, compatibility projections, and runtime-bundle heads.
