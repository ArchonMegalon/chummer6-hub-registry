...
Verified guardrail commit: 061cc27, Tighten M101 helper proof casing guard
...
Repo-local guardrail commit `061cc27` is now pinned in the machine-readable proof receipt and verifier so future shards also prove the current M101 repeat-prevention guardrail floor includes case-insensitive canonical registry, Fleet queue staging, and design queue staging active-run helper proof negative cases, not only the original implementation commit.
...
The machine-readable proof receipt now records `verified_guardrail_commit: 061cc27`, and the package-specific verifier rejects active-run helper or telemetry proof markers case-insensitively in canonical successor registry, Fleet queue staging, design queue staging, the proof receipt, and this closeout note. That prevents mixed-case task-local telemetry or similar operator-run artifact names from keeping this completed package green. The previous verified guardrail floor `d8f3911` is superseded by `061cc27`.
...
* `scripts/verify_next90_m101_registry_promotion_discipline.py` can no longer resolve the recorded verified guardrail commit `061cc27`,
