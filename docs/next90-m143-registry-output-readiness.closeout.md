...
- The same materializer now consumes Fleet's published `FLAGSHIP_PRODUCT_READINESS.generated.json` receipt. If desktop readiness is stale, failing, or still missing `desktop_client` coverage, registry freshness flips stale and public or signed-in release and exchange shelves downgrade instead of claiming current output truth.
