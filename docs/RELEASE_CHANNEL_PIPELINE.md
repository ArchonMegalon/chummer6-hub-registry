...
`releaseProof` is required and verifier-bound for promoted shelf truth, and `releaseProof.status` must be passing (`pass`, `passed`, or `ready`) so failed or missing proof packets cannot be promoted as shelf truth.
