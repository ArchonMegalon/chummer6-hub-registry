...
`desktopTupleCoverage.externalProofRequests` rows are strict verifier-bound contract truth for cross-host blocker capture (`tupleId`, `head`, `platform`, `rid`, `requiredHost`, `requiredProofs`, `expectedArtifactId`, `expectedInstallerFileName`, `expectedPublicInstallRoute`, `expectedStartupSmokeReceiptPath`, `startupSmokeReceiptContract`, and `proofCaptureCommands`) and must match canonical tuple-derived values exactly.
