...
`releaseProof.journeysPassed` and `releaseProof.proofRoutes` are verifier-bound evidence lists: both must be present, non-empty, string-only, and duplicate-free (after normalization), so promoted shelf truth always carries concrete multi-journey and route-level provenance instead of empty placeholders.
