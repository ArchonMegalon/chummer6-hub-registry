...
`releaseProof.journeysPassed` and `releaseProof.proofRoutes` are verifier-bound evidence lists: both must be present, non-empty, string-only, and duplicate-free (after normalization). `journeysPassed` entries must be canonical journey-id tokens (`[a-z0-9][a-z0-9_-]*`), and each `proofRoutes` entry must be a slash-led route path (for example `/downloads/install/...`), so promoted shelf truth always carries concrete multi-journey and route-level provenance instead of empty or free-form placeholders.
