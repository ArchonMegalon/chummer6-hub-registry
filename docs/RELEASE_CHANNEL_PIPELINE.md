...
Select this profile with the exact required-platform request `linux,windows,macos` and provide `--macos-flagship-evidence <path>`. Registry requires real, non-symlink download and startup-smoke directories, opens all three installer files as regular files, and verifies their size and SHA-256 against the candidate. It requires exactly one duplicate-free native startup receipt for each canonical Linux, Windows, and macOS tuple, with the exact candidate release version, and rejects missing directories, extra receipts, skipped proof, and every smoke-filter bypass. The macOS input is the duplicate-free UTF-8 `chummer6-ui.macos-flagship-evidence` v3 aggregate. Pretty JSON and its final newline are accepted because Registry binds the SHA-256 and byte length of the exact source file; semantic validation rejects missing or extra evidence fields, unsafe reference paths, candidate-byte drift, non-passing signing/Gatekeeper/stapling facts, non-accepted notarization, or GitHub provenance that does not bind the global candidate source commit.

The only cross-channel promotion is an explicit protected-assembly boundary. A `preview` candidate may become `public_stable` only when `--channel-promotion-authority <path>` names a duplicate-free `chummer6-ui.global-flagship-channel-promotion-authority.v1` document produced on protected UI `main` in the `global-flagship-publication-input-assembly` environment. Registry byte-validates its candidate, proposal, final receipt, three role-keyed approvals, three Hub topology receipts, three startup receipts, and destination intent; recomputes the compact six-field artifact inventory digest; and requires `registryProjectionAuthorized: true` with `publicationMutationAuthorized: false`. The resulting `chummer.registry.global-flagship-channel-promotion` binding is embedded identically in both projections. No generic channel matcher can turn a preview execution receipt into publication authority.
...
* Preview normally uses `contractName: "chummer.preview-release-decision/v1"`, top-level `releaseVersion`, `releaseDecisionStatus` (`review_required` or `preview_ready`), equal `status`, and `manifestSha256`. It also requires exact snapshot/manifest scope bindings for `channel`, `registryCommit`, sorted `platforms`, sorted `primaryHeadByPlatform`, canonical manifest-derived `fallbackHeadsByPlatform`, `supportOwner`, and `artifactAccessClass`, plus string fields `authoritySnapshotSha256`, `candidateDecisionStatus`, and `candidateDecisionSha256`. A raw manifest-bound `review_required` seed may set all three candidate fields to empty strings. `preview_ready` requires lowercase 64-hex snapshot/candidate digests and those values must match the exact prior `CURRENT.json` candidate.
* `chummer.preview-release-decision/v2` is a narrow, review-only public-byte handoff. It is emitted only for exactly one approved Windows/Avalonia/win-x64 installer whose approved scope is `open_public` with `preview_unsigned_allowed`, whose source publication binding is still `preview`, and whose manifest remains `published` / `public_release_review_required` / `review_required`. Its exact `artifactHandoff` object binds the approved-scope digest, release and artifact tuple, immutable byte digest and size, download/install routes, access class, signing posture, and source publication state. This makes the approved preview bytes publicly handoff-capable without claiming `preview_ready`, stable, flagship, supportable, or generally published availability. V1 behavior is unchanged, generic `publicationState: "preview"` remains ineligible, and v2 can never carry a ready verdict.
...
* the manifest artifact is an `installer`, is `compatible`, has a lowercase SHA-256, positive size, and a credential/query/fragment/traversal-free immutable root-relative `/downloads/g/<generationId>/files/<fileName>` URL;
...
* exactly one matching publication binding is `published` with `publicationScope: "signed-in-and-public"`, a public shelf ref, and the same tuple, channel, release, and public install route; the only exception is the exact scope-approved v2 Windows review-byte handoff described above, which admits source state `preview` without upgrading the release decision beyond `review_required`;
...
For the current Chummer6 preview transaction, `desktopTupleCoverage.requiredDesktopPlatforms` is an exact mutable release target in canonical order: `linux`, `windows`. Materialization and strict verification do not derive, shrink, or widen that target from artifacts or stale source configuration. The only allowed artifact rows are the Avalonia Linux x64 installer and Avalonia Windows x64 installer; duplicate rows, Blazor bytes, other RIDs, other media kinds, unknown platforms, and macOS rows fail closed before proof filtering. A Linux-only or Windows-only in-scope payload reports the other required tuple as missing, `complete=false`, `rolloutState=coverage_incomplete`, and `supportabilityState=review_required`. A current macOS artifact payload or a required-platform list that retains macOS is rejected as stale scope. macOS remains a recognized buildable platform and its historical proof remains valid as separate lane evidence, but it is not part of this release candidate.
...
| --- | --- | --- | --- | --- | --- |
| `avalonia:linux:linux-x64` | `primary` | `promoted` | `manual_recovery_required` | `not_revoked` | `none` |
| `blazor-desktop:linux:linux-x64` | `fallback` | `proof_required` | `fallback_not_promoted` | `not_revoked` | `none` |
...
`requiredDesktopHeads` remains `["avalonia"]` because preview completion is judged on the one primary head. That does not erase route truth: fallback `blazor-desktop` rows stay mandatory in `desktopRouteTruth` for the current Linux and Windows tuples so rollback, recovery, revoke, and install posture remain explicit without publishing fallback bytes.
